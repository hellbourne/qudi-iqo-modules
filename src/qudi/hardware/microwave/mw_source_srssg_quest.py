...
class MicrowaveSRSSG(MicrowaveInterface):
    ...
    def cw_power(self):
        ...
        """
        with self._thread_lock:
            return float(self._device.query(':SOUR:POW:POW?'))
    ...
    def cw_frequency(self):
        ...
        """
        with self._thread_lock:
            return float(self._device.query('SOUR:FREQ:CW?'))
    ...
    def set_cw(self, frequency, power):
        ...
        with self._thread_lock:
            ...
            if self._is_vector_sg:
                ...
                self._write('STYP 0')
            self._write(f'SOUR:FREQ:CW {frequency:e}')
            self._write(f':SOUR:POW:POW {power:f}')
