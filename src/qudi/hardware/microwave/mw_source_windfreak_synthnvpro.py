...
Adapted from the Synth HD Pro code with major changes
"""
...
class MicrowaveSynthNVPro(MicrowaveInterface):
    ...
    def on_activate(self):
        ...
        # for the SynthNV, the "do not append anything when writing is crucial"
        ...
        self.model = self._conn.query('+')[0:-1]
        self.sernr = self._conn.query('-')[0:-1]
        self.mod_hw = self._conn.query('v1')[0:-1]
        self.mod_fw = self._conn.query('v0')[0:-1]

        self.log.info('Found {0} Ser No: {1} {2} {3}'.format(
            self.model, self.sernr, self.mod_hw, self.mod_fw)
        )

        tmp = self._conn.query('z')[0:-1]
    ...
    def off(self):
        ...
        """
        self._conn.write('g0')  # this also switches off the sweep
    ...
    def get_input_power(self):
        """ Gets the microwave input power as detected in the RF-in port

        @return float: the power measured/detected at the device in dBm
        """
        self._conn.write('w1')
        end_of_message = True
        while end_of_message:
            tmp = self._conn.read()[0:-1]
            if tmp == 'EOM.':
    ...
    def cw_on(self):
        ...
        self.current_output_mode = MicrowaveMode.CW
        status = self._on()
        self.log.debug('On: {}'.format(status))
        return 0
    ...
    def set_mod(self, on=False):
        self.log.warn('Windfreak SynthNV Pro does not support MOD mode')
        return 0
    ...
    def set_sweep(self, start=None, stop=None, step=None, power=None):
        ...
        if (start is not None) and (stop is not None) and (step is not None):
            ...
            self._conn.write('X0')  # 0 : linear, 1 : tabular
            # self._conn.write('c0')  # 0 : single, 1 : continuous
            ...
            # sweep direction
            if stop >= start:
                self._conn.write('^1')  # sweep direction is up
            else:
                self._conn.write('^0')  # sweep direction is down

            # sweep frequency and steps
            self._conn.write('l{0:5.7f}'.format(start / 1e6))  # SynthNV expects MHz for frequency
            self._conn.write('u{0:5.7f}'.format(stop / 1e6))   # SynthNV expects MHz for frequency
            self._conn.write('s{0:5.7f}'.format(step / 1e6))   # SynthNV expects MHz for frequency

        # sweep power - right now power sweep is NOT implemented
        if power is not None:
            self._conn.write('W{0:2.3f}'.format(power))
            self._conn.write('[{0:2.3f}'.format(power))
            self._conn.write(']{0:2.3f}'.format(power))
    ...
    def set_ext_trigger(self, pol, dwelltime):
        ...
        @param TriggerEdge pol: polarization of the trigger (basically rising edge or falling edge)
        @param dwelltime: minimum dwell time [SynthNV units are millisecond]
    ...
    def _stat(self):
        ...
        """
        E = float(self._conn.query('E?'))
        r = float(self._conn.query('r?'))
        h = float(self._conn.query('h?'))
        return E, r, h
