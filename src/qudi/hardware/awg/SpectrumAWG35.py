...
class AWG():
    ...
    def write_sequence_step(self, step_index, mem_segment_index, loops, goto, next_condition):
        ...
        for card in self.cards:
            res = card.write_sequence_step(step_index, mem_segment_index, loops, goto, next_condition)
            # print('write seq error ', res)
    ...
    def set_selected_channels(self, channels):
        ...
        if input is not None:
            self.cards[0].set_selected_channels(input)
        ...
        input = c2_channels
        if input is not None:
            self.cards[1].set_selected_channels(input)
...
class Card():
    ...
    def set_memory_size(self, mem_size, is_seq_segment=False):
        ...
        if is_seq_segment:
            if mem_size < 192:
                print('minimum segment size is 192, must be at least 129ns')
